...
// Create SQUINT using Nevilles
//Vinayak Gargya
void  SQUINTquadsNevilles(PNT A, PNT B, PNT C, PNT D, PNT[] Point, float time,int n)
{
  int i=0;
  n = n * 4;
  A.setTo(NevilleCubicPNT(0f,Point[i],0.33,Point[i+4],0.66,Point[i+8],1f,Point[i++ + 12],time));
  B.setTo(NevilleCubicPNT(0f,Point[i],0.33,Point[i+4],0.66,Point[i+8],1f,Point[i++ + 12],time));
  C.setTo(NevilleCubicPNT(0f,Point[i],0.33,Point[i+4],0.66,Point[i+8],1f,Point[i++ + 12],time));
  D.setTo(NevilleCubicPNT(0f,Point[i],0.33,Point[i+4],0.66,Point[i+8],1f,Point[i++ + 12],time));
}
//Vinayak and Amita
...
//similarities
 void SQUINTMap(PNT A, PNT B, PNT C, PNT D,int n,PImage pix1,PImage pix2){
