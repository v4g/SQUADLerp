...
void makeFourSquares() // arranges 16 control points into 4 neat squares. For CS6491 and possibly for the advanced part of CS3451
  ...
  for(int i=0; i<4; i++)
    ...
    Point[4*i+0]=P(O); Point[4*i+0].moveBy(-d*error,-d*error);  Point[4*i+1]=P(O); Point[4*i+1].moveBy(d,-d*error); 
...
void fourEdges()
{
  String edgesFile = "FourEdges.pts";
  println("loadingpoints from file: " + edgesFile);
  String subpts;
}
